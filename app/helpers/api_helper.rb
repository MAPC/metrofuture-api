module ApiHelper
  ...
  def first_page
    send("#{ @klass }_path", "page[number]" => 1, "page[size]" => page_size)
  end

  def previous_page(current_page_num)
    return nil if current_page_num <= 1
    send("#{ @klass }_path", "page[number]" => current_page_num - 1, "page[size]" => page_size)
  end

  def current_page(current_page_num)
    send("#{ @klass }_path", "page[number]" => current_page_num, "page[size]" => page_size)
  end
  ...
  def last_page(last_page_num)
    send("#{ @klass }_path", "page[number]" => last_page_num, "page[size]" => page_size)
  end
